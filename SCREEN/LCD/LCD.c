...
{							    
//写十六位GRAM
  u8 i,RGB_CodeH,RGB_CodeL;
	RGB_CodeH=(RGB_Code>>8)&0xff;
  RGB_CodeL=RGB_Code&0xff;	
	LCD_CS_LOW()
	MOSI_HIGH()//RS=1 数据
	CLK_LOW()		  
	CLK_HIGH()
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	CLK_HIGH();
		...
		for(i=0; i<8; i++)
		{
			CLK_LOW();		r=r << 1 | HAL_GPIO_ReadPin(SPI_MISO_GPIO_Port,SPI_MISO_Pin); 	
			CLK_HIGH();			
		}
		...
		for(i=0; i<8; i++)
		{
			CLK_LOW();		g=g << 1 | HAL_GPIO_ReadPin(SPI_MISO_GPIO_Port,SPI_MISO_Pin);
			CLK_HIGH();			
		}
		...
		for(i=0; i<8; i++)
		{
			CLK_LOW();		b=b << 1 | HAL_GPIO_ReadPin(SPI_MISO_GPIO_Port,SPI_MISO_Pin); 	
			CLK_HIGH();			
		}		
